...
const deriveCredentialState = (params: {
...
function MaidLoginContent() {
  ...
  const [status, setStatus] = useState<LoginStatus>(
    derivedCredentialState.status,
  );
  const [credentials, setCredentials] = useState<MaidCredentials | null>(
    derivedCredentialState.credentials,
  );
  const [maidProfile, setMaidProfile] = useState<Maid | null>(null);
  const [errorMessage, setErrorMessage] = useState<string | null>(
    derivedCredentialState.error,
  );
  const [setupFormName, setSetupFormName] = useState("");
  const [setupFormImageFile, setSetupFormImageFile] = useState<File | null>(
    null,
  );
        ...
        <Card className="w-full max-w-md border-rose-100/80 bg-white/90 text-center shadow-lg backdrop-blur">
          ...
          <CardContent className="space-y-5 text-left">
            ...
            {shouldShowErrorPanel && (
              <Alert
                variant="destructive"
                className="rounded-md border border-red-200 bg-red-50/70 px-4 py-3"
              >
                <AlertTitle className="text-xs font-semibold tracking-wide text-red-500">
                  エラー
                </AlertTitle>
            ...
            {shouldShowStatusPanel && (
              <Alert className="rounded-md border border-dashed border-blue-100/80 bg-blue-50/40 px-4 py-3">
                <AlertTitle className="text-xs font-semibold tracking-wide text-blue-400">
                  {statusLabel}
                </AlertTitle>
              </Alert>
            )}
